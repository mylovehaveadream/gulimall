...
import com.guigu.gulimall.search.feign.ProductFeignService;
import com.guigu.gulimall.search.service.MallSearchService;
import com.guigu.gulimall.search.vo.AttrResponseVo;
import com.guigu.gulimall.search.vo.BrandVo;
...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            ...
            //3.分析响应数据封装成我们需要的格式
            result = buildSearchResult(response,param);    //构建检索的结果
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private SearchRequest buildSearchRequrest(SearchParam param) {
        ...
        //1.2.构建bool - filter - 按照三级分类id来查询的
        if(param.getCatalog3Id() != null){
            boolQuery.filter(QueryBuilders.termQuery("catalogId",param.getCatalog3Id()));
        }

        //1.2.构建bool - filter - 按照品牌的id来查询的
        if(param.getBrandId() != null && param.getBrandId().size() > 0){
            boolQuery.filter(QueryBuilders.termsQuery("brandId",param.getBrandId()));
        }

        //1.2.构建bool - filter - 按照所有指定的属性来查询的
        ...
        //1.2.构建bool - filter - 按照库存是否有来查询的
        if(param.getHasStock() != null){
            boolQuery.filter(QueryBuilders.termQuery("hasStock",param.getHasStock()==1));
        }

        //1.2.构建bool - filter - 按照价格区间来查询的
        if(!StringUtils.isEmpty(param.getSkuPrice())){
            ...
            if(s.length == 2){
                ...
                if(param.getSkuPrice().startsWith("_")){    //以什么开始的 _500
                    rangeQuery.lte(s[0]);
                }

                if(param.getSkuPrice().endsWith("_")) {      //以什么结尾 500_
                    rangeQuery.gte(s[0]);
                }
            }
        ...
        //2.3.高亮
        if(!StringUtils.isEmpty(param.getKeyword())){   //只有传了keyword的模糊匹配查询条件以后，才有必要写高亮
        ...
        System.out.println("构建的DSL"+sourceBuilder.toString());//打印DSL的语句
    ...
    private SearchResult buildSearchResult(SearchResponse response,SearchParam param) {
        ...
        //1.返回的所有查询到的商品
        SearchHits hits = response.getHits();   //获取命中的记录
        ...
        if(hits.getHits() != null && hits.getHits().length > 0){
            for (SearchHit hit : hits.getHits()) {
                ...
                SkuEsModel esModel = JSON.parseObject(sourceAsString, SkuEsModel.class);//将字符串转成指定的对象
                //设置高亮的内容
                if(!StringUtils.isEmpty(param.getKeyword())){
        ...
        for (Terms.Bucket bucket : attr_id_agg.getBuckets()) {
            ...
            List<String> attrValues = ((ParsedStringTerms) bucket.getAggregations()
                    .get("attr_value_agg")).getBuckets().stream().map(item -> {
        ...
        //4.当前所有商品涉及到的所有分类信息
        ParsedLongTerms catalog_agg = response.getAggregations().get("catalog_agg");//这样拿到了分类的聚合信息
        ...
        //7.分页信息-总页码-计算 11(总页码)/2(每页两个) = 5..1
        ...
        for (int i=1;i<=totalPages;i++) {
            pageNavs.add(i);    //可以遍历的页码
        }
        result.setPageNavs(pageNavs);


        //8.构建面包屑导航功能,面包屑导航这一部分的只是导航筛选属性，其他的不导航，检索条件和分类它是固定的
        if(param.getAttrs() != null && param.getAttrs().size() > 0){
            List<SearchResult.NavVo> collect = param.getAttrs().stream().map(attr -> {
                //1.分析每个attrs传过来的查询参数的值
                ...
                result.getAttrIds().add(Long.parseLong(s[0]));  //拿到当前的属性
                if(r.getCode() == 0){
        ...
        if(param.getBrandId() != null && param.getBrandId().size() > 0){
            ...
            if(r.getCode() == 0){
                ...
                for (BrandVo brandVo : brand) {
                    ...
                    replace = repalceQueryString(param, brandVo.getBrandId()+"","brandId");
                }
    ...
    private String repalceQueryString(SearchParam param, String value,String key) {
        String encode = null;
        try {
            encode = URLEncoder.encode(value, "UTF-8");  //需要进行中文编码，编码以后才能进行字符串的替换
